...
function getEnvValue(ctx: any, name: string): string | undefined {
  ...
  const fromProcess = (globalThis as any)?.process?.env?.[name] ?? process.env?.[name];
  return fromProcess;
}
...
export const onPost: RequestHandler = async (ctx) => {
  ...
  try {
    ...
    const email =
      body.customerEmail ? String(body.customerEmail) :
    ...
    const okUrl = 'https://www.11book.online/payment/success';
    const failUrl = 'https://www.11book.online/payment/fail';
    const callbackUrl = 'https://www.11book.online/api/maib/callback';

    const orderId = String(body.orderId ?? globalThis.crypto?.randomUUID?.() ?? `${Date.now()}`);
    ...
    if (!tokenRes.ok || !tokenData?.ok || !accessToken) {
      ...
      json(500, { error: 'MAIB token error', status: tokenRes.status, details: tokenData });
      return;
    }
    ...
    const { res: payRes, data: payData } = await fetchJson(
      `${MAIB_API}/pay`,
      {
        ...
        body: JSON.stringify({
          ...
          amount,
          currency,
          description: body.description ? String(body.description) : '11book.online',
          language,
          orderId,
          email,
          callbackUrl,
    ...
    if (!payRes.ok || !payData?.ok || !payUrl) {
      ...
      json(500, { error: 'MAIB pay error', status: payRes.status, details: payData });
      return;
    }
